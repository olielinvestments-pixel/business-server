...
app.get('/api/debug-rows', async (req, res) => {
    try {
          const client = await serviceAuth.getClient();
          const sheets = google.sheets({ version: 'v4', auth: client });
          const resp = await sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: ORDERS_SHEET_NAME + '!A:I' });
...
async function writeConfirmationLinkToOrders(sheets, orderNum, docLink) {
  try {
    ...
    await sheets.spreadsheets.values.update({ spreadsheetId: SHEET_ID, range: ORDERS_SHEET_NAME + '!I' + targetRow, valueInputOption: 'RAW', requestBody: { values: [[docLink]] } });
